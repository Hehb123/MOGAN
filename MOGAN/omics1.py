...
def omics1(update,omics1,omics2,adj_file,label,flag):
    ...
    if update>1:
      ...
      protein = np.array(protein).astype(np.float32).transpose()
    ...
    for epoch in range(num_epochs):
        for n, (real_samples, _) in enumerate(mRNA_train_loader):
            ...
            protein_train_data = protein[:,n*batch_size:(n+1)*batch_size]
            protein_train_data=torch.from_numpy(protein_train_data)

            latent_value = torch.matmul(protein_train_data.t(),adj)
