...
def prediction(mRNA_value,GAN_epoch,labels,update=None):
  ...
  if update==1:
    print('AUC for real omics3:', np.mean(AUC_all,axis=0))
  '''
  if GAN_epoch==0:
...
def omics3(update,omics1,omics3,adj_file,label):
    ...
    xy, x_ind, y_ind = np.intersect1d(mRNA.columns,meth.columns,return_indices=True)
    _, x_ind1, y_ind1 = np.intersect1d(meth.index,adj.columns,return_indices=True)
    xy1, x_ind2, y_ind2 = np.intersect1d(mRNA.index.values,adj.index.values,return_indices=True)

    mRNA = mRNA.iloc[:,x_ind]
    meth = meth.iloc[:,y_ind]
    ...
    if update>1:
        ...
        meth = pd.read_csv(meth_file_name,delimiter=',',index_col=0)
