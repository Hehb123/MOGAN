  ...
  for col in range(labels.shape[1]):
    ...
    for i in range(trial):
      ...
      for train_index, test_index in skf.split(X, y):
          ...
          auc= KNN(X_train, y_train, X_test, y_test)
          AUC.append(auc)
...
def omics2(update,omics1,omics2,adj_file,label,flag):
    print('Generating protein update '+str(update))


    mRNA = load_data(omics1)
    protein = pd.read_csv(omics2,index_col=0,delimiter=',')
    adj = pd.read_csv(adj_file,index_col=0)
    xy, x_ind, y_ind = np.intersect1d(mRNA.columns,protein.columns,return_indices=True)
    _, x_ind1, y_ind1 = np.intersect1d(protein.index,adj.columns,return_indices=True)
    xy1, x_ind2, y_ind2 = np.intersect1d(mRNA.index.values,adj.index.values,return_indices=True)

    mRNA = mRNA.iloc[:,x_ind]
    protein = protein.iloc[:,y_ind]
    mRNA = mRNA.iloc[x_ind2,:]
    protein = protein.iloc[x_ind1,:]
    adj = adj.iloc[:,y_ind1]
    adj = adj.iloc[y_ind2,:]
    mRNA = mRNA.fillna(0)
    ...
    mRNA = mRNA.iloc[:,x_ind]
    protein = protein.iloc[:,x_ind]
    y= data.iloc[y_ind,:].astype(str)
    #y[y=='Positive']=1
    #y[y=='Negative']=0
    labels=np.array(y).astype(np.float32)

    sample_name = protein.columns
    feature_name = protein.index

    mRNA = np.array(mRNA).transpose().astype(np.float32)
    protein = np.array(protein).transpose().astype(np.float32)
    adj = np.array(adj).astype(np.float32)

    X_0 = torch.from_numpy(protein).to(device)

    if update>1:
        mRNA_file_name = 'omics1_'+str(update-1)+'.csv'
        protein_file_name = 'omics2_'+str(update-1)+'.csv'
        if flag == 1:
           protein_file_name = 'omics3_'+str(update-1)+'.csv'
        protein = pd.read_csv(protein_file_name,delimiter=',',index_col=0)
        protein = np.array(protein).astype(np.float32)
        mRNA = pd.read_csv(mRNA_file_name,delimiter=',',index_col=0)
        mRNA = np.array(mRNA).astype(np.float32)

    n_input_protein=np.size(protein,1)
    sample_size = np.size(protein,0)

    C=np.sqrt(np.outer(np.sum(np.absolute(adj),0),np.sum(np.absolute(adj),1)))
    adj=np.divide(adj,C.transpose())

    protein_train_data=torch.from_numpy(protein)
    adj=torch.from_numpy(adj)

    protein_train_labels = torch.zeros(sample_size)

    protein_train_set = [(protein_train_data[i], protein_train_labels[i]) for i in range(sample_size)]

    batch_size=sample_size
    protein_train_loader = torch.utils.data.DataLoader(protein_train_set, 
                                  batch_size=batch_size, shuffle=False)
           
    discriminator = Discriminator(n_input_protein).to(device)
    generator = Generator(n_input_protein).to(device)
    ...
    optimizer_discriminator = torch.optim.RMSprop(discriminator.parameters(), lr=lr_D)  #RMS优化器
    ...
    for epoch in range(num_epochs):
        for n, (real_samples, _) in enumerate(protein_train_loader):
